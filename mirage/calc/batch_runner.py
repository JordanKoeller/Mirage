...
class BatchRunner:
  ...
  def start(self):
    ...
    engine_process = Process(
      name="EngineProcess", target=BatchRunner._engine_main, args=(self.simulation, send))

    engine_process.start() # This starts the engine in a separate process
    evt = recv.recv_blocking()
    if evt.closed or evt.empty:
      logger.info("EngineProcess returned code indicating the thread closed. No result is saved.")
    else:
