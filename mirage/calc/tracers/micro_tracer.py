...
class MicrolensingRayTracer(RayTracer):
  ...
  def trace(self, rays: PixelRegion) -> u.Quantity:
    ...
    logger.info(
      "Running with %d starts, M_tot=%.2f M_sun" % (stars_mass.shape[0], np.sum(stars_mass.value)))
    logger.info(f"Running with {pixels.shape} (Total={pixels.shape[0]*pixels.shape[1]}) pixels")
