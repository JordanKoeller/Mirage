...
from .InitialMassFunction import Evolved_IMF, IMF_broken_powerlaw
...
class AnimatedMassFunction(MassFunction):
  ...
  def generate_stars(self, region: Region, mass_density: u.Quantity, center_on_zero=False) -> np.ndarray:
    if len(self._stars) == 0:
      ...
      self._injection_sigma = region.radius * \
          self._velocity_characteristics[1] / (self._velocity_characteristics[0]**2)
  ...
  def _prune_and_replenish(self):
    ...
    if approx_injected > 0:
      add_times = rng.normal(self._injection_mean.value,
                             self._injection_sigma.value, num_injected + 10)
  ...
  def _get_velocities_for(self, stars):
    ...
    rng_mag = np.sqrt(velocity_directions[:, 0]**2 +
                      velocity_directions[:, 1]**2 + velocity_directions[:, 2]**2)
