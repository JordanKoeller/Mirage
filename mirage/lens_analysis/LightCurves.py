...
from scipy.stats import anderson_ksamp, energy_distance, ks_2samp, mannwhitneyu
  ...
  for ind in range(int(len(lc1) - 1)):
    ...
    for peak in peak_batch:
      try:
        ...
        err += 1
  print("Accumulated %d errors of %d total. Error rate of %.2f percent" %
        (err, len(ret_shifts) + err, 100 * err / ((len(ret_shifts) + err))))
  if with_peaks:
...
def bucket_and_clean(dataset):
  ...
  for k in buckets.keys():
    ...
    buckets[k] = {'mean': mean.flatten(), 'std': std.flatten(),
                  'asym': k / 100, 'num': arr.shape[0]}
  return buckets
...
class LightCurve(object):
  ...
  def get_events(self, threshold=0.8 / u.uas, smoothing_factor=1.1 * u.uas, min_separation=u.Quantity(5.0, 'uas'), require_isolation=False):
    slice_list = self.get_event_slices(
      threshold, smoothing_factor, min_separation, require_isolation)
    ret = []
...
class Event(object):
  ...
  def prominences_above(self, cutoff: float) -> int:
    ...
    # argrelmax(self._data[self._parent_index],order=4))[0]
    candidates = np.array([i for i in range(0, len(self._data[self._parent_index]))])
    proms = len(list(filter(lambda x: prominence(
      self._data[self._parent_index], x) > cutoff, candidates)))
    return proms
...
class EventClassificationTable(object):
  ...
  def __init__(self, events, group_count):
    ...
    def get_ind(asym): return int(round((asym - min_sep) / dx))
    errors = 0
    for event in events:
      try:
