'''
lens_analysis is a module focused on analyzing and processing data produced by |mirage|.
It includes functions for parsing result files, visualizing magnification maps and light curves,
and statistically analysing light curve and magnification map data.
'''

from .CausticMap import CausticMap
from .LightCurves import (Event, EventClassificationTable, LightCurve,
                          LightCurveBatch, LightCurveSlice)
...
def write(data, filename):
  ...
  if isinstance(data, MicrolensingParameters):
    ...
    raise ValueError("type " + str(type(data)) +
                     " not recognized as a valid type to write to file.")
  fm.open(filename)
  fm.write(data)
  fm.close()
