...
class SingularIsothermalSphereLens(LensingSystem):
  ...
  def einstein_radius(self) -> u.Quantity:
    d_ls = self.cosmology.angular_diameter_distance_z1z2(self.redshift, self.quasar.redshift)
    d_s = self.cosmology.angular_diameter_distance(self.quasar.redshift)
    ret = (4 * pi * self.velocity_dispersion.to('m/s')**2*d_ls)/(d_s*const.c**2)
    ret = ret.to('')
  ...
  def _convergence(self, position: Vec2D) -> float:
    ...
    return b/(2*sqrt(q**2*t1**2 + t2**2)) + 3*g*cos(E - p + arctan2(t2,t1))/4
  ...
  def _shear(self, position: Vec2D) -> float:
    ...
    g = self.shear.r.value
    q = 1 - self.ellipticity.r.value
    return sqrt(t1**2*(2*b*t2*(t1**2 + t2**2)*sqrt(q**2*t1**2 + t2**2) + \
      ...
      cos(E - p + arctan2(t2,t1))/4 + g*q**2*t1**3*t2*sin(E - p + arctan2(t2,t1)) - \
      g*q**2*t1**2*t2**2*cos(E - p + arctan2(t2,t1))/4 + g*t1**2*t2**2*cos(E - p + \
      arctan2(t2,t1))/4 + g*t1*t2**3*sin(E - p + arctan2(t2,t1)) - g*t2**4*cos(E - p + \
      arctan2(t2,t1))/4)**2/(q**2*t1**4 + q**2*t1**2*t2**2 + t1**2*t2**2 + t2**4)**2)