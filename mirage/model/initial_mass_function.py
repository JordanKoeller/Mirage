...
Provides the code for sampling star masses from an initial mass function.

The two main classes to use are Imf and ImfBrokenPowerlaw

This file contains the "bare bones" functions and classes.
...
class Imf:
    ...
    def __eq__(self, other) -> bool:
        return (
            ...
            and (self._mass_limits == other._mass_limits).all()
            and self._multi_props == other._multi_props
            and self._rng_seed == other._rng_seed
        )
    ...
    def generate_cluster(self, total_mass) -> np.ndarray:
        """
        ...
        if self._mass_limits[-1] > total_mass:
            self._mass_limits[-1] = total_mass

        # Estimate the mean number of stars expected.
        self.normalize(total_mass)  # type: ignore
        ...
        while total_massTally < total_mass:
            ...
            newMasses = self.dice_star_cl(uniX)  # type: ignore
            ...
            if self._multi_props is not None:
                ...
                compMasses = np.empty((len(newMasses),), dtype=np.object)  # type: ignore
                compMasses.fill([])  # type: ignore
                ...
                newIsMultiple = (
                    self.random_number_generator.rand(int(newStarCount)) < MF
                )
                ...
                if old:
                    ...
                    for ii in range(len(newMasses)):
                        if newIsMultiple[ii]:
                            ...
                            n_comp = 1 + self.random_number_generator.poisson(
                                (CSF[ii] / MF[ii]) - 1
                            )

                            # Determine the mass ratios of the companions
                            q_values = self._multi_props.random_q(
                                self.random_number_generator.rand(n_comp)
                            )
            ...
            total_massTally += newTotalMassTally
        ...
        # total mass.
        idx = np.abs(massCumSum - total_mass).argmin()
    ...
    def calc_multi(
        self, newMasses, compMasses, newSystemMasses, newIsMultiple, CSF, MF
    ):
        """
        ...
        for ii in num:
            ...
            if ii == 1:
                ...
                q_values = self._multi_props.random_q(
                    self.random_number_generator.rand(len(tmp))
                )
                ...
                q_values = self._multi_props.random_q(
                    self.random_number_generator.rand(len(tmp), ii)
                )
...
class ImfBrokenPowerlaw(Imf):
    ...
    def __eq__(self, other) -> bool:
        return (
            isinstance(self, type(other))
            and (self._mass_limits == other._mass_limits).all()
            and (self._powers == other._powers).all()
            and self._multi_props == other._multi_props
            and self._rng_seed == other._rng_seed
        )
    ...
    def xi(self, m):
        ...
        """
        returnFloat = isinstance(m, float)
    ...
    def prim_xi(self, a):
        """ """
        returnFloat = isinstance(a, float)
    ...
    def dice_star_cl(self, r):
        ...
        """
        returnFloat = isinstance(r, float)
        ...
        # -----For i = 1 --> n, where n is the number of intervals?
...
class AgedIMFBrokenPowerLaw(ImfBrokenPowerlaw):
    def __init__(
        self,
        distro: ImfBrokenPowerlaw,
        conversions: dict[float, float],
    ) -> None:
        """
...
class MillerScalo1979(ImfBrokenPowerlaw):
    def __init__(self, rng_seed: Optional[int] = None):
        ImfBrokenPowerlaw.__init__(
            self,
            mass_limits=[0.1, 1.0, 10.0, np.inf],
            powers=[-1.4, -2.5, -3.3],
            rng_seed=rng_seed,
        )
...
def prim_power(m, power):
    ...
    """
    returnFloat = isinstance(m, float) and isinstance(power, float)
...
def inv_prim_power(x, power):
    ...
    when power == -1.
    """
    returnFloat = isinstance(x, float) and isinstance(power, float)
...
def log_normal(m, mean_logm, sigma_logm):
    returnFloat = (
        isinstance(m, float)
        and isinstance(mean_logm, float)
        and isinstance(sigma_logm, float)
    )
