...
class Imf:
  ...
  -  companionMasses (:class:`numpy.ndarray`) : List of companion masses.
    ...
    while totalMassTally < totalMass:
      ...
      if self._multi_props != None:
        ...
        newIsMultiple = self.random_number_generator.rand(
          int(newStarCount)) < MF
        ...
        if old:
          ...
          for ii in range(len(newMasses)):
            if newIsMultiple[ii]:
              ...
              n_comp = 1 + \
                self.random_number_generator.poisson(
                  (CSF[ii] / MF[ii]) - 1)
  ...
  def calc_multi(self, newMasses, compMasses, newSystemMasses, newIsMultiple, CSF, MF):
    ...
    n_comp_arr = 1 + \
      self.random_number_generator.poisson((CSF[idx] / MF[idx]) - 1)
    ...
    for ii in num:
      ...
      if ii == 1:
        ...
        for jj in range(len(tmp)):
          ...
          compMasses[idx[tmp[jj]]] = m_comp_tmp[m_comp_tmp >=
                              self._mass_limits[0]]
          newSystemMasses[idx[tmp[jj]]
                  ] += compMasses[idx[tmp[jj]]].sum()
...
class IMFSalpeter1955(ImfBrokenPowerlaw):
  def __init__(self):
    ImfBrokenPowerlaw.__init__(self,
                               mass_limits=[0.4, 10.0],
                               powers=[-2.3])
