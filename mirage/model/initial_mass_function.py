        ...
        while totalMassTally < totalMass:
            ...
            if self._multi_props != None:
                ...
                if new:
                    ...
                    # t1 = time.time()
                    compMasses, newSystemMasses, newIsMultiple = self.calc_multi(
                        newMasses, compMasses, newSystemMasses, newIsMultiple, CSF, MF
                    )
...
def log_normal(m, mean_logm, sigma_logm):
    returnFloat = (
        (type(m) == float)
        and (type(mean_logm) == float)
        and (type(sigma_logm) == float)
    )
