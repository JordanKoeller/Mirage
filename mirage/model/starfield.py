...
class Starfield:
  ...
  @cache
  def get_starfield(
      self, total_mass: u.Quantity, region_radius: u.Quantity) -> Tuple[u.Quantity, u.Quantity]:
    num_stars = total_mass / 0.5
    masses = u.Quantity(
      self.initial_mass_function.generate_cluster(total_mass.to('solMass').value), 'solMass')
    masses = u.Quantity([0.5 for i in range(int(num_stars.value))], 'solMass')
    num_stars = len(masses)

    positions: np.ndarray = np.ndarray((num_stars, 2), dtype=np.float64) # Buffer where each row is [mass, x, y]
    ...
    random_thetas = 2 * pi * self.initial_mass_function.random_number_generator.rand(num_stars)
