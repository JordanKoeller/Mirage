...
from mirage.util import Jsonable, Region, Vec2D

from .Parameters import MicrolensingParameters, Parameters, ParametersError
...
class Simulation(Jsonable):
  ...
  def parameters(self) -> MicrolensingParameters:
    if self.trial_variance:
      nspace = {}
      try:
        exec(self.trial_variance, {'old_parameters': self.original_parameters,
                                   'trial_number': self.trial_number, 'u': u, 'np': np, 'copy': copy, 'math': math}, nspace)
