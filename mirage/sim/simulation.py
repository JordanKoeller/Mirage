...
class Simulation(ABC):
  ...
  def from_dict(sim_dict: dict) -> Self: # type: ignore
    ...
    macro_fields = {Dictify._to_pascal_case(f.name) for f in fields(MacrolensingSimulation)}
    micro_fields = {Dictify._to_pascal_case(f.name) for f in fields(MicrolensingSimulation)}
    ...
    if present_micro_fields:
      ...
      raise ValueError("Tried to construct a MicrolensingSimulation but got None instead")
