...
class Variance:
    ...
    def from_string(
        ...
        string_value: str,
        default_tag: str,
        edge_behavior=EdgeBehavior.FIXED_TAIL,
    ) -> "Variance":
        for regex_exp, func in EXPR_REGEXES:
            ...
            if matches:
                ...
                edge = (
                    EdgeBehavior[edge_group.strip()]
                    if edge_group
                    else edge_behavior
                )
                variance_string = matches[0]
                return Variance(
                    variance_string, list(func(variance_arguments)), tag, edge
                )
...
class VariancePreprocessor:
    def __init__(
        self, default_edge_behavior: EdgeBehavior = EdgeBehavior.FIXED_TAIL
    ):
    ...
    def generate_variants(self, yaml_document: str) -> list[str]:
        ...
        if all_variances:
            ...
            return self._generate_all_documents(
                yaml_document, variances, max_variance_length
            )
    ...
    def _generate_all_documents(
        self,
        yaml_document: str,
        variances: dict[str, list[Variance]],
        values_length: int,
    ) -> list[str]:
        """
        ...
        permutations = self._generate_variants_values(
            len(variances), values_length
        )

        for permutation in permutations:
            documents.append(
                self._generate_document(yaml_document, permutation, variances)
            )

        return documents

    def _generate_variants_values(
        self, num_tags: int, max_value: int
    ) -> list[list[int]]:
        """
        ...
        for i, t in enumerate(sorted_tags):
            for variance_obj in variances[t]:
                template = template.replace(
                    variance_obj.variance_string,
                    str(variance_obj.get_value(indices[i])),
                )
        return template

    def _get_all_variances(
        self, yaml_document: str
    ) -> dict[str, list[Variance]]:
        ...
        for line in yaml_document.splitlines():
            try:
                variance = Variance.from_string(
                    line,
                    f"zzzzzzzzz_MIRAGE_DEFAULT_TAG_{tag_seed}",
                    self.default_edge_behavior,
                )
                tag_seed += 1
            ...
            except ValueError:  # No variance present. Do nothing.
                pass
