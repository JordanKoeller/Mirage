...
class Dictify:
    ...
    def _dataclass_from_dict(klass: Type[T], dict_obj: Dict[str, Any]) -> T:
        ...
        expected_fields = {
            Dictify._to_pascal_case(f.name): f
            for f in fields(klass)  # type: ignore
        }
    ...
    def _get_custom_serializer(
        ...
        for ser in Dictify.__custom_serializers:
            if isclass(instance_or_type) and issubclass(
                instance_or_type, ser.value_type
            ):
                return ser
        ...
        if get_origin(klass) is list:
            ...
            if isabstract(inner_type):
                ...
                for elem in dictable_value:
                    ...
                    concrete_type = DelegateRegistry.get_typedef(
                        inner_type, concrete_type_name
                    )
                    if concrete_type is not None:
                        values.append(
                            Dictify._value_from_dict(
                                concrete_type, elem[concrete_type_name]
                            )
                        )
                    else:
                        raise ValueError(
    ...
    def _dictify_dataclass(obj: Any):
        ...
        for field in obj_fields:
            ...
            if get_origin(field.type) == list:
                ...
                if isabstract(inner_type) and DelegateRegistry.has_supertype(
                    inner_type
                ):
