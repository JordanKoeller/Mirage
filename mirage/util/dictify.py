...
class Dictify:
  ...
  def to_dict(value: Any) -> Any:
    ...
    if isinstance(value, (datetime, time, date)):
      return str(value)
    raise ValueError(f"Encountered undictifiable value {value} of type {str(type(value))}")
  ...
  def _get_custom_serializer(
      instance_or_type: Union[T, Type[T]]) -> Optional[CustomSerializer[T]]:
  ...
  def _is_python_collection(type_def: Type) -> bool:
    return type_def in (list, dict) or \
      str(type_def).startswith('typing.Dict') or \
      str(type_def).startswith('typing.List')
      ...
      if isabstract(inner_type): 
        ...
        for elem in dictable_value:
          ...
          if concrete_type is not None:
            values.append(Dictify._value_from_dict(concrete_type, elem[concrete_type_name]))
          else:
