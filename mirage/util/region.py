...
class Region:
  ...
  def outline(self) -> u.Quantity:
    center: Vec2D = self.center.to(self.unit) if self.center else Vec2D.zero_vector(self.unit)
...
class PixelRegion(Region):
  ...
  def pixels(self) -> u.Quantity:
    ...
    x_ax = np.linspace(coords_start.x.value, coords_end.x.value, int(self.resolution.x.value))
    y_ax = np.linspace(coords_start.y.value, coords_end.y.value, int(self.resolution.y.value))
