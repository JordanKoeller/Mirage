...
class PolarVec(BaseRepresentation, DictifyMixin):
  ...
  def __init__(self, r: u.Quantity, theta: u.Quantity):
    try:
      ...
      raise ValueError(f"`theta` must be some type of angle, but found units {theta.unit}")
  ...
  def to_dict(self) -> Dict[str, Any]:
    return {
      'R': Dictify.to_dict(self.r),
      'Theta': Dictify.to_dict(self.theta)
    }
  ...
  def unit_vectors(self):
    ...
    theta = self.theta.to('rad').value
    return {
      'r': CartesianRepresentation(np.cos(theta) * self.unit, np.sin(theta) * self.unit, unit_zero),
      'theta': CartesianRepresentation(-np.sin(theta) * self.unit, np.cos(theta) * self.unit, unit_zero),
    }
