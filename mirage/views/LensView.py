...
from . import ImageCurveView, get_renderer
...
class AnimationController(object):
  ...
  def __init__(self, simulation: 'AnimationSimulation', engine: 'AbstractEngine'):
    ...
    self._process = Process(target=_processThreadFunc, args=(
      self._dt, self._simulation, self._engine, self._queue, self._sender), daemon=True)
    self._process.start()
...
class LensView(ImageCurveView):
  ...
  def on_press(self, event):
    if event.inaxes != self._imgAx or event.button != 3 or self._playing:
      return
  ...
  def on_motion(self, event):
    ...
    if self.press is None:
      return
    if event.inaxes != self._imgAx or event.button != 3:
      return
