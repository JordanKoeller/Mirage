...
class MagnificationMapView(ImageCurveView):
  ...
  def on_press(self, event):
    ...
    if event.inaxes != self.axes or event.button != 3:
      return
  ...
  def display(self, magmap: MagnificationMap = None):
    ...
    img = self.axes.imshow(magmap.data.T, cmap=self._cmap,
                           extent=[-x, x, -y, y], vmin=-maxVal, vmax=maxVal)
  ...
  def show_curve(self, curve):
    ...
    # NOTE: I need to transpose to be consistent with theview
    self.line.set_ydata([start[1].value, end[1].value])
