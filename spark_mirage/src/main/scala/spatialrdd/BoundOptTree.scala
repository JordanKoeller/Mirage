...
class BoundOptTree[A <: RayBank](values:A, branchSize: Int) extends SpatialData {
  ...
  class Node(val indI: Int, val indJ: Int, var split: Double) extends Serializable {
    val (maxX, minX, maxY, minY) = extremesFinder(values,indI,indJ)
    ...
    def containedBy(x:Double, y:Double ,r2:Double):Boolean = {
      distance(x,minX,y,minY) < r2 && distance(x,minX,y,maxY) < r2 &&
      distance(x,maxX,y,minY) < r2 && distance(x,maxX,y,maxY) < r2
    }

    private def distance(x1:Double,x2:Double,y1:Double,y2:Double):Double = {
      val dx = x2 - x1
      val dy = y2 - y1
      dx*dx+dy*dy
    }
  ...
  def searchNodes(x: Double, y: Double, r: Double): Int = {
    ...
    while (!toSearch.isEmpty) {
      ...
      if (boxes(searching).containedBy(x,y,r2)) counter += boxes(searching).size //This is the contained_by statement. ****************************************
  ...
  private def extremesFinder(arr:A,s:Int,e:Int) = {
    var ret = (Double.MinValue, Double.MaxValue, Double.MinValue, Double.MaxValue) //maxX, minX, maxY, minY
    for (i <- s until e) {
      val (x,y) = (arr.x(i),arr.y(i))
      ret = (math.max(ret._1,x),math.min(ret._2,x),math.max(ret._3,y),math.min(ret._4,y))
    }
    ret
  }
}
...
object BoundOptTree {
  ...
  def apply[A <: RayBank](values:A):BoundOptTree[A] = {
    new BoundOptTree(values,binSize)
  }
