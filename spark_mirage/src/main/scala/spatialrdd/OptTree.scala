...
class OptTree[A <: RayBank](values: A, branchSize: Int) extends SpatialData {
  ...
  class Node(val indI: Int, val indJ: Int, var split: Double)
    ...
    def search(
      ...
      var aggregator = ResultZero
      for (i <- indI until indJ) {
        if (collector.properParity(values, i) && values.sourceX(i) > x1 && values
              .sourceX(i) < x2 && values.sourceY(i) < y2 && values.sourceY(i) > y1) {
          aggregator += collector(values, i)
        }
      }
      aggregator
  ...
  def searchNodes(
    ...
    while (!toSearch.isEmpty) {
      ...
      if (false)
        counter += boxes(searching).size //This is the contained_by statement. ****************************************
      //Case 2: box overlaps the circle
        ...
        if (searching * 2 + 1 >= boxes.size) {
          ...
          if (boxes(searching).overlapsLeft(x, y, r, level))
            toSearch = (searching * 2 + 1) :: toSearch
          if (boxes(searching).overlapsRight(x, y, r, level))
            toSearch = (searching * 2 + 2) :: toSearch
        }
      }
    }
    counter
    ...
    while (!toSearch.isEmpty) {
      ...
      //Case 2: box overlaps the circle
        ...
        if (searching * 2 + 1 >= boxes.size) {
          ...
          //Case 2: Need to go into its children. Check each individually, see if needs to be added.
          if (boxes(searching).overlapsLeft(x1, x2, y1, y2, level))
            toSearch = (searching * 2 + 1) :: toSearch
          if (boxes(searching).overlapsRight(x1, x2, y1, y2, level))
            toSearch = (searching * 2 + 2) :: toSearch
        }
      }
    }
  ...
  private def list_constructNodes(): Array[Node] = {
    val tmpBoxes: collection.mutable.ArrayBuffer[Node] =
      collection.mutable.ArrayBuffer[Node](new Node(0, size, 0.0))
    var level = 0
    var flag = true
    var index = 0
    while (flag) {
      if (tmpBoxes(index).size <= branchSize) flag = false
        ...
        for (i <- 0 until math.pow(2, level).toInt) {
          ...
          tmpBoxes.append(
            new Node(tmpBoxes(index).indI, tmpBoxes(index).indI + k, 0.0)
          )
          tmpBoxes.append(
            new Node(tmpBoxes(index).indI + k, tmpBoxes(index).indJ, 0.0)
          )
          index += 1
        }
        level += 1
  ...
  private def selectX(k: Int, n: Int, start: Int): Double = {
    ...
    while (true) {
      if (ir <= l) {
        if (ir == l + 1 && values.sourceX(ir + start) < values.sourceX(
              (l + start)
            )) values.swap(l + start, ir + start)
        return values.sourceX((k + start))
      } else {
        mid = (l + ir) >> 1
        values.swap(mid + start, l + 1 + start)
        if (values.sourceX((l + start)) > values.sourceX((ir + start)))
          values.swap(l + start, ir + start)
        if (values.sourceX((l + 1 + start)) > values.sourceX((ir + start)))
          values.swap(l + 1 + start, ir + start)
        if (values.sourceX((l + start)) > values.sourceX((l + 1 + start)))
          values.swap(l + start, l + 1 + start)
        i = l + 1
        j = ir
  ...
  private def selectY(k: Int, n: Int, start: Int): Double = {
    ...
    while (true) {
      if (ir <= l) {
        if (ir == l + 1 && values.sourceY((ir + start)) < values.sourceY(
              (l + start)
            )) values.swap(l + start, ir + start)
        ...
        mid = (l + ir) >> 1
        values.swap(mid + start, l + 1 + start)
        if (values.sourceY((l + start)) > values.sourceY((ir + start)))
          values.swap(l + start, ir + start)
        if (values.sourceY((l + 1 + start)) > values.sourceY((ir + start)))
          values.swap(l + 1 + start, ir + start)
        if (values.sourceY((l + start)) > values.sourceY((l + 1 + start)))
          values.swap(l + start, l + 1 + start)
        i = l + 1
        j = ir
        a = values.getTuple(l + 1 + start)
