...
class RDDGrid[A <: RayBank: ClassTag, SD <: SpatialData: ClassTag](rdd: RDD[SD])
    extends RDDGridProperty {

  def queryPointsFromGen(
      gen: GridGenerator,
      radius: Double,
      sc: SparkContext,
    ...
    while (iter.hasNext) {
      ...
      val queries = rdd.flatMap { grid =>
        ...
        for (i <- 0 until broadcasted.value.size) {
          ...
          val num = grid.searchNodes(
            qloc._1,
            qloc._2,
            radius,
            broadcasted.value.rayCollector
          )
          if (num != 0) rett ::= (i, num)
        }
        rett
  ...
  private def collect(
      data: RDD[(Long, Result)],
      accumulator: Array[Array[Result]]
  ): Unit = {
    val reduced = data.reduceByKey((acc, cnt) => acc + cnt).map { elem =>
      new IndexPair(elem._1) -> elem._2
    }
    val collected = reduced.collect
    collected.foreach { elem =>
      accumulator(elem._1.x)(elem._1.y) += elem._2
    }
  }
    ...
    val queries = rdd.flatMap { grid =>
      ...
      for (i <- 0 until queryPts.value.length) {
        if (grid.intersects(
          ...
          val num = grid.query_point_count(
            queryPts.value(i)._1,
            queryPts.value(i)._2,
            r.value
          )
...
object RDDGrid {
  ...
  def apply[A <: RayBank: ClassTag, SD <: SpatialData: ClassTag](
      data: RDD[A],
      partitioner: SpatialPartitioning = new BalancedColumnPartitioner,
      nodeStructure: A => SD
  ): RDDGrid[A, SD] = {
    val ret = data
      .map(arr => nodeStructure(arr))
