...
class ArrayQueryIterator(qpts: Array[Array[DoublePair]])(
  ...
  def nextBatch(): ArrayLocalQueryIterator = {
    ...
    new ArrayLocalQueryIterator(
      qpts.slice(dispatchedRows.head, dispatchedRows.last + 1).flatten
    )
  }
...
class ArrayLocalQueryIterator(values: Array[DoublePair])(
    implicit val rayCollector: RayCollector
) extends LocalQueryIterator {
  def size: Int = values.size
  def apply(i: Int): DoublePair = values(i)
  def computeRay(rayBank: RayBank, index: Int): Double =
    rayCollector(rayBank, index)
}
