...
from mirage.sim import MacrolensingSimulation
...
class TestEngine(TestCase):
  def setUp(self):
    self.lens = PointLens(
      quasar=Quasar(redshift=2),
      redshift=0.5,
      mass=u.Quantity(1e12, "solMass"),
    )

    float(self.lens.einstein_radius.to("arcsec").value)

    self.simulation = MacrolensingSimulation(
      lensing_system=self.lens,
      reducers=[
        reducers.LensedImageReducer(
          query=Vec2D(0, 0, "arcsec"),
          radius=u.Quantity(0.01, "arcsec"),
        )
      ],
