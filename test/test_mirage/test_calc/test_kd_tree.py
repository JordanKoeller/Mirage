...
class TestKdTreePerformance(TestCase):
    ...
    def testKdTreeQuery(self):
        ...
        self.timeit(
            lambda: pytree.query_rays(self.pos, self.radius), "Py.query"
        )
        rstree = RustKdTree(self.dataset)
        self.timeit(
            lambda: rstree.query_rays(self.pos, self.radius), "Rs.query"
        )
    ...
    def getQueryTime(self, size: int, Tree) -> float:
        ...
        tree = Tree(dataset)
        return self.timeit(
            lambda: tree.query_rays(
                Vec2D(0.2, 0.1, "arcsec"), u.Quantity(0.06, "arcsec")
            ),
