...
class TestMicrolensingSimulation(TestCase):
    ...
    def testGetRayBundle_success(self):
        ...
        for expected_ratio in axis_ratios:
            ...
            self.assertAlmostEqual(
                resolution.x.value * resolution.y.value,
                self.sim.ray_count,
                0.005,
            )
    ...
    def testGetRayBundle_table(self):
        ...
        ax.set_title("Pcnt Error vs (ray_count, axis_ratio)")
    ...
    def assertAlmostEqual(self, first, second, pcnt_diff):
        ...
        if pcnt > pcnt_diff:
            self.fail(
                f"Error of {pcnt} between {first} and {second} exceeds {pcnt_diff}"
            )
