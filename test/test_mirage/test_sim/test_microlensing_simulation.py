...
class TestMicrolensingSimulation(TestCase):
  ...
  def setUp(self):
    self.sim = MicrolensingSimulation(
      lensing_system=PointLens(quasar=Quasar(2.0), redshift=0.5, mass=Quantity(1e12, 'solMass')),
  ...
  def testGetRayBundle_success(self):
    ...
    for expected_ratio in axis_ratios:
      ...
      self.assertAlmostEqual(resolution.x.value * resolution.y.value, self.sim.ray_count, 0.005)
